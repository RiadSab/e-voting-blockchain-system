...
pub fn prove(vote: u64, beta: &Fr, c1: &G, c2: &G, pubkey: &G) -> OrProof {
    ...
    if vote == 0 {
        ...
        let mut b0 = Branch { a1: g * r, a2: *pubkey * r, c: Fr::from(0u64), z: Fr::from(0u64) };
        ...
        let r = Fr::rand(&mut rng);
        let mut b1 = Branch { a1: g * r, a2: *pubkey * r, c: Fr::from(0u64), z: Fr::from(0u64) };
...
pub fn verify(proof: &OrProof, c1: &G, c2: &G, pubkey: &G) -> bool {
    ...
    let m0 = *c2;
    let ok0 = g * b0.z == b0.a1 + *c1 * b0.c
        && *pubkey * b0.z == b0.a2 + m0 * b0.c;

    // 3. branch 1 (point = c2 - g)
    let m1 = *c2 - g;
    let ok1 = g * b1.z == b1.a1 + *c1 * b1.c
        && *pubkey * b1.z == b1.a2 + m1 * b1.c;
